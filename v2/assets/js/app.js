/* ==========================================================================
   Alison Amorim — Editorial · interações (restraint over spectacle)
   ========================================================================== */
(function () {
  'use strict';
  ...
  function applyStaticLang() {
    ...
    document.querySelectorAll('.lang-switch [data-lang]').forEach(function (o) {
  ...
  function buildFigures() {
    var g = $('#figures'); g.innerHTML = '';
    D.stats.forEach(function (s) {
      var f = el('div', 'figure reveal');
      var suf = s.suffix ? '<sup>' + s.suffix + '</sup>' : '';
      f.innerHTML = '<div class="fig-num">' + s.value + suf + '</div>' +
        '<div class="fig-label">' + esc(s[lang]) + '</div>';
      g.appendChild(f);
    });
  }
  ...
  function buildLedger() {
    ...
    D.experience.forEach(function (e) {
      ...
      row.innerHTML =
        ...
        '<ul class="lr-bullets">' + bullets + '</ul>' +
  ...
  function buildAll() {
    buildFigures(); buildLedger(); buildCaps(); buildWorks(); buildWriting(); buildCreds();
    observeReveals();
    hydrateStars();
  }
  ...
  $('#tocToggle').addEventListener('click', function () {
    tocOpen = !tocOpen;
    var all = $('#tocAll'); all.hidden = !tocOpen;
    this.textContent = tocLabel();
    if (tocOpen) observeReveals();
  });

  /* ---------- nav active ---------- */
  var ids = ['work', 'capabilities', 'selected', 'writing', 'contact'];
  function onScroll() {
    var cur = '';
